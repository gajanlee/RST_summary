...
def combineparse2sent(sent, parse):
    ...
    tokenlist = list(map(lambda x: x.decode(), tokenlist))
    ...
    with open(fconll, 'w') as fout:
        for sent in sentlist:
            for token in sent.tokenlist:
                ...
                line = str(sent.idx) + '\t' + str(token.idx) + '\t' + token.word.decode() + '\t' + token.lemma.decode() + '\t' + token.pos.decode() + '\t' + token.deptype + '\t' + str(token.headidx) + '\t' + token.nertype.decode() + '\t' + token.partialparse.decode() + '\n'
